#!/bin/usr/env deno run --allow-run --allow-env
...
function notify(message: string) {
  ...
  // send message
  console.log(`\x1b]1337;SetUserVar=claude=${base64Message}\x07`)
}
...
const main = async () => {
  ...
  try {
    ...
    if (data.hook_event_name === "Stop") {
      notify(`display notification "Task Completed 🚀" with title "⚡ Claude Code" subtitle "${repoInfo} 📦"`,)
    } else if (data.hook_event_name === "Notification") {
      notify(`display notification "Awaiting Confirmation 🔔" with title "⚡ Claude Code" subtitle "${repoInfo} 📦"`)
    } else {
    ...
    notify(`display notification "Hook Failed !" with title "Claude Code Error 🚨"`)
  }
}
